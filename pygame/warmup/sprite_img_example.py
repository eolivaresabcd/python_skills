...
game_folder = os.path.dirname(__file__) # Get the directory of the current file
img_folder = os.path.join(game_folder, 'img') # Create a path to the img folder

# CREATE PLAYER CLASS
class Player(pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self) # Initialize the sprite class
        image = pygame.image.load(os.path.join(img_folder, 'bugs.jpg')).convert_alpha() # Load the player image
            # the convert_alpha() method is used to convert the image to a format that is faster to draw
        self.image = pygame.transform.scale_by(image, 0.1) # Scale the image to 10% of its original size
        self.image.set_colorkey(BLACK) # Set the color key for the image to the color of the scren, making it transparent
